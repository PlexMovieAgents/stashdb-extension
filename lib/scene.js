    ...
    chrome.runtime.sendMessage({msg: "queryLocalStash", query: { query, variables: { id } } }, results => {
      ...
      if(id in ignoredScenes) {
        ...
        if(results.data.findScenes.scenes.length) {
          ...
          let max_height = 0;
          for(let scene of results.data.findScenes.scenes) {
            for(let file of scene.files) {
          ...
          } else if(max_height > 900) {
            new_image.attr('src', chrome.runtime.getURL('images/blue_check.svg'));
          } else {
            new_image.attr('src', chrome.runtime.getURL('images/yellow_check.svg'));
          }
        } else {
          new_image.attr('src', chrome.runtime.getURL('images/red_x.svg'));
          let svgUrl = chrome.runtime.getURL('images/red_x.svg');
          fetch(svgUrl)
